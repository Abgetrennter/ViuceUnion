...
class DataManager(object):
    ...
    def load_plugin_info(self):
        ...
        for plugin in nonebot.plugin.get_loaded_plugins():
            json_path: Path = store.get_data_file("menu_config", f"{plugin.name}.json")
            if json_path.exists():
                try:
...
class TemplateManager(object):
    def __init__(self):
        ...
        self.templates_path = store.get_config_dir("menu_config")/ "template"  # 模板路径
        self.load_templates()
...
class MenuManager(object):  # 菜单总管理
    def __init__(self):
        self.cwd = store.get_config_dir("menu_config")
    ...
    def config_folder_make(self):
        if not (self.cwd / "fonts").exists():
            (self.cwd / "fonts").mkdir()
        if not (self.cwd / "templates").exists():
            (self.cwd/ "templates").mkdir()
        if not (self.cwd  / "menus").exists():
            (self.cwd / "menus").mkdir()
        if not (self.cwd  / "config.json").exists():
            with (self.cwd  / "config.json").open(
                "w", encoding="utf-8"
            ) as fp:
